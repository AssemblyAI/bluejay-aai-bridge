...
- Conversation content, transcripts and tool arguments alike, goes through `spoken()` and is gated on `LOG_TRANSCRIPTS`, which is off unless set. It is untrusted text on its way to a log line: control characters are stripped so a caller cannot speak a fake log entry, and the length is bounded. The code cannot tell a terminal from a hosted bridge feeding a third-party aggregator, so it assumes the second; `.env.example` opts local runs back in.
